...
class XCarriage(element.Primitive):
    ...
    def _construction(self):
        ...
        bottom.location = [self.topLength/2-self.bottomLength/2,
                           self.rodSpacing-1,
                           0]
        ...
        for idx, tab in enumerate(tabs):
            if idx in [3, 4]:
                tab = operation.Rotate(axis=[0, 0, 1],
                                       angle=180,
                                       elements=[tab])
...
class YBearingMount(component.Element):
    def __init__(self, rodDiameter=8,
        ...
        self.yRodMount = YRodMount(rodDiameter=rodDiameter, mountLength=mountLength, stepper=stepper, beltSize=beltSize,
                                   beltSeperation=self.beltSeperation, beltWidth=beltWidth, holeDiameter=holeDiameter)
        self.bearingHoldRadius = self.bearing.outerDiameter/2 + self.belt.height + 2
        self.bearingHoldHeight = (self.yRodMount.height - self.yRodMount.beltPass[2])/2
        self.construction = self._construction()

    def _construction(self):
        asm = self.yRodMount
#        upperBearingHold = dShape(radius=self.bearingHoldRadius,
#                                  extension=self.bearingHoldRadius,
#                                  length=self.bearingHoldHeight)
#        lowerBearingHold = copy.deepcopy(upperBearingHold)
#        upperBearingHold.location = [self.yRodMount.width/2,
#                                     -self.bearing.outerDiameter/2-1,
#                                     self.yRodMount.beltPass[2]+self.bearingHoldHeight]
#        lowerBearingHold.location = [self.yRodMount.width/2,
#                                     -self.bearing.outerDiameter/2-1,
#                                     0]
#        hole = element.Hole(radius=self.bearing.innerDiameter/2,
#                            height=self.yRodMount.height+0.2)
#        hole.location = copy.copy(lowerBearingHold.location)
#        hole.location[2] = -0.1
#        asm = operation.Union([asm, upperBearingHold, lowerBearingHold]) - hole
        return asm


class BeltClamp(component.Element):
    def __init__(self, thickness=5, screw="M3", xCarriage=None):
        component.Element.__init__(self, name="yrodmount")
        self.holeSpacing = xCarriage.beltClampHoleSpacing
        self.screw = hardware.Nut(size=screw)
...
class YRodMount(component.Element):
    def __init__(self,
        ...
        leftX = self.width/2-self.beltSeperation/2-self.beltPass[0]-self.holeDiameter/2
        rightX = self.width/2+self.beltSeperation/2+self.beltPass[0]+self.holeDiameter/2
    ...
    def _construction(self):
        ...
        #rod encasement
        rodEn = dShape(radius=self.rodEncasementDiameter/2,
                                 extension=self.rodDiameter/2+self.plateThick,
                                 length=self.length)
        rodEn = operation.Rotate(axis=[1, 0, 0], angle=-90, elements=[rodEn])
        rodEn.location = [self.width/2, 0, self.rodCenter]
        asm = rodEn
        #rod
        rod = element.Hole(radius=self.rodDiameter/2, height=self.length)
        ...
        #Mounting holes
        for i in range(0, 4):
            holeTab = dShape(radius=self.tabRadius,
                             extension=self.width/2,
                             length=self.plateThick)
            holeTab = operation.Translate(location=self.mountingHoles[i],
                                          elements=[holeTab])
            hole = element.Hole(radius=self.holeDiameter/2,
                                height=self.plateThick+0.2)
            hole.location = self.mountingHoles[i]
            asm = asm + holeTab - hole
        return asm
...
class LinearBearingHolder(component.Element):
    def __init__(self,
        ...
        self.height = self.lb.outerDiameter * 0.70 + self.wall + zipTieHeight
        self.construction = self._construction()
